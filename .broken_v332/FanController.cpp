...
// ============================================================
//  FanController — V3.32
//    - No temp curve (fan is boost-only)
//    - Safety override at 45°C
//    - DS18B20 keeps last good value on bad read
//    - 3-tier alarm: Warning / Oh Shit / Kill
// ============================================================
...
void readDS18B20(float &currentTemp) {
    ...
    if (tempConversionRunning && now - tempConversionStart >= 750) {
        ...
        if (t == DEVICE_DISCONNECTED_C || t == 0.0 || t < -50.0f || t > 100.0f) {
            Serial.printf("[DS18B20] bad read: %.1f — keeping last value\n", t);
            return;
        }
...
void updateFanAndAlerts(
    ...
    // ---- 3-tier alert classification ----
    int newLevel = 0;
    if      (currentTemp >= config.alarmKill)    newLevel = 3;
    else if (currentTemp >= config.alarmPanic)   newLevel = 2;
    ...
    if (newLevel != alertLevel) {
        ...
        lastAlertStart = millis() - 120000;   // fire immediately
    }

    // ---- Phone gate ----
    bool effectivePhone = (config.phoneMode == "off") ? true : phonePresent;

    // ---- Fan decision: boost OR safety override ----
    int newPwm = 0;
    ...
    // Safety override at alarmWarning
    if (currentTemp >= config.alarmWarning) {
        newPwm = 255;
    }
